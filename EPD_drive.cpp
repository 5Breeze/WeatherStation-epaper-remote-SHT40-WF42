...
int Duck_EPD::getIcon(int weathercodeindex) {
 ...
 void Duck_EPD::DrawWeatherChart(int xmin,int xmax,int ymin,int ymax,int point_n,int show_n,String tmax,String tmin,String code_d,String code_n,String text_d,String text_n,String date,String week)//绘制天气温度变化曲线
 {
  if(tmax==",,,,,")
  {
    tmax="5,5,5,5,5,5";
 ...
 for(int i=0;i<show_n;i++)
  {    
     ...
     else
     {
      ...
      SetFont(ICON32);DrawUTF(xmax+14,ymin+dy*i-16,String(char(getIcon(code_d_a[i].toInt()))));
      SetFont(FONT12);DrawUTF(xmin-28,ymin+dy*i-3*date_a[i].length(),date_a[i]);
...
{ 
   ...
   else if(EPD_Type==WF29BZ03||EPD_Type==WF32)
   {
      if(frame==0)
      {
      if(x<(xDot*2)&&y<(yDot*2))  EPDbuffer[x/4+y*xDot/4]&=~(0x80>>((x%4)*2)) ;
      }
      else
      {
      if(x<(xDot*2)&&y<(yDot*2))  EPDbuffer[x/4+y*xDot/4]&=~(0x40>>((x%4)*2)) ;
      }
    }
   else
    {
...
{	
	...
	if(EPD_Type==OPM42)
 {
  ...
  if(EPD_Type==DKE42_3COLOR||EPD_Type==DKE29_3COLOR)
 {
  EPD_Init();
  EPD_WriteCMD(0x21); 
  EPD_WriteData(0x00);
...
void Duck_EPD::EPD_Dis_Full(unsigned char *DisBuffer,unsigned char Label)
{
  ...
  //EPD_WriteDispRam_Old(xDot/8, yDot, (unsigned char *)DisBuffer,0,1); 
	EPD_Update();	
  ReadBusy_long();ReadBusy_long();ReadBusy_long();
  EPD_WriteDispRam_Old(xDot/8, yDot, (unsigned char *)DisBuffer,0,1);  
  }
